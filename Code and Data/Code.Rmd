---
title: "Green Bubbles: A Four-Stage Paradigm for Detection and Propagation (resubmission 2.0)"
...
#RMSE BSADF test
round(Metrics::rmse(c(matrix_ADF[, 1], matrix_ADF[, 2], matrix_ADF[, 3]), c(
  rep(40, correct_ADF),
  rep(60, correct_ADF),
  rep(70, correct_ADF)
)), 3)
...
The same analysis is conducted for the abrupt or sudden path.
...
All series are integrated of order one, I(1), and the Johansen procedure indicates one cointegrating relationship ($r = 1$) at the 1% significance level and two at the 5% significance level (Table 3 in the article). The VECM results indicate that both Renixx.dl1 and OFR.dl1 significantly affect OFR, suggesting that these variables are important short-term drivers of the dependent variable (Table 4 in the article).
...
The following chunks are dedicated to analyzing the dynamics of bubbles in the RENIXX index. For this analysis, the following new libraries are required (please ensure they are installed before running the code).
...
*ARIMAX (with exogenous variables at time t)

*ARIMAX (with exogenous variables at time t-1)
...
```{r, warning = FALSE}
#Granger causality (multivariate). In the paper, the reported value is the p-value corresponding to the F-statistic (first table).
...
#MCS
set.seed(3)
Loss <- (predictions_uni[, c(1, 2, 3, 4)] - real_data)^2
MCSprocedure(Loss,
             alpha = 0.15,
             B = 5000,
             statistic = "TR")
